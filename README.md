...
### `requirements.txt` (Julia-oriented)

This isn’t used by Julia directly (Julia relies on `Project.toml`/`Manifest.toml`), but it’s a convenient summary of the Julia environment and can be referenced in docs or CI.

```text
# Julia version
julia>=1.12

# Julia packages required for host-aware modelling and synthetic data generation
DifferentialEquations
Sundials
JLD2
YAML
